...
\f0\fs24 \cf0 # Get started with Claude Code hooks\
\
...
\
Select `+ Add new matcher\'85` to run your hook only on Bash tool calls.\
\
...
### Step 3: Add the hook\
\
Select `+ Add new hook\'85` and enter this command:\
\
```bash  theme=\{null\}\
    ...
    if os.path.exists(file_path):\
        ...
        if formatted != content:\
            with open(file_path, 'w', encoding='utf-8') as f:\
                f.write(formatted)\
            print(f"\uc0\u10003  Fixed markdown formatting in \{file_path\}")\
    \
