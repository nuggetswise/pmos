...
### Setup\
\
Runs when Claude Code is invoked with repository setup and maintenance flags (`--init`, `--init-only`, or `--maintenance`). Use this hook for operations you don't want on every session\'97such as installing dependencies, running migrations, or periodic maintenance tasks.\
\
<Note>\
...
<Warning>\
  ...
  exits with code 2 (blocking error), `stderr` text is used directly\'97any JSON in `stdout`\
...
if issues:\
    for message in issues:\
        print(f"\'95 \{message\}", file=sys.stderr)\
